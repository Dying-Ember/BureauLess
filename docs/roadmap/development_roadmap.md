...
fixture, API, and maintained-demo scopes. That milestone line established
advisor outcome learning, routing/dispatch decision artifacts, bounded context,
context telemetry, and the first narrow `codex-cli` worker path.
...
Provider expansion remained outside that remediation scope.

The current external audit harness has since expanded the same canonical
session boundary to registered adapters for Codex CLI, Claude Code, Gemini CLI,
OpenCode, and Pi. It records route contracts, native evidence, workspace facts,
independent verification, decision points, side-effect coverage, benchmark
identity, append-only observations, and provenance-aware paired comparisons.
The exact Agent×Provider state is generated by `bureauless agent matrix
--evidence`; dated endpoint results remain in `docs/audits/`.
...
REX-001 was closed in Runtime M4, and Runtime M5 closed provider-side telemetry
attribution on the maintained Codex Responses path. Provider-attributed
telemetry is not universal: other adapters retain their native Agent-reported
usage/cost provenance, and missing currency evidence remains unavailable.
...
1. Publish and repeat identity-matched audit cohorts across the implemented
   Agent routes; prefer stronger evidence over adding more Agent names.
2. Implement a missing route adapter only when both the runtime contract and a
   usable endpoint instance can be verified. OpenCode Anthropic and
   OpenCode/Pi Responses remain explicit gaps, not implied compatibility.
3. Keep provider-attributed and Agent-reported telemetry separate; do not rank
   cost or tokens across runs whose provenance is not comparable.
4. Add a Workbench audit view only after the append-only observation and
   comparison artifacts prove stable through real use.
5. Keep BureauLess outside Agent model/tool loops and out of provider gateway,
   secret-broker, fallback-mesh, and frontend-owned write authority roles.
